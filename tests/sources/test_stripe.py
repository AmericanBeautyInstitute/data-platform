...
from sources.stripe.extract import (
    Raw,
    Record,
    _date_to_timestamp,
    _to_raw,
...
def test_extract_composes_fetch_parse_to_table(raw_rows):
    ...
    with patch("sources.stripe.extract.fetch", return_value=raw_rows):
...
def test_fetch_calls_charges_list(mock_client):
    ...
    with patch(
        "sources.stripe.extract._to_raw", side_effect=lambda c: _to_raw(dict(c))
    ):
...
def test_fetch_empty_response_returns_empty_list():
    ...
    with patch("sources.stripe.extract._to_raw", side_effect=lambda c: RAW_ROW_1):
