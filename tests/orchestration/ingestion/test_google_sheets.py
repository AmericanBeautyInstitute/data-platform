...
from orchestration.defs.ingestion.google_sheets import (
    SHEET_NAMES,
...
def test_materialize_gcs_source_includes_sheet_name(env_vars, google_sheets_resource):
    ...
    with (
        patch(
            f"{_ING}.google_sheets.sheets_extract.extract",
            return_value=SAMPLE_TABLE,
        ),
        patch(f"{_ING}.google_sheets.gcs_load.load", mock_gcs_load),
        patch(f"{_ING}.google_sheets.bq_load.load", return_value=FAKE_ROWS_LOADED),
        ...
        patch(
            f"{_ING}.resources.GoogleSheetsResource.get_client",
...
def test_materialize_output_metadata_keys(env_vars, google_sheets_resource):
    ...
    with (
        ...
        patch(f"{_ING}.google_sheets.gcs_load.load", return_value=FAKE_GCS_URI),
        patch(f"{_ING}.google_sheets.bq_load.load", return_value=FAKE_ROWS_LOADED),
        patch("dagster_gcp.GCSResource.get_client", return_value=MagicMock()),
        patch("dagster_gcp.BigQueryResource.get_client", return_value=MagicMock()),
        patch(
...
def test_materialize_passes_sheet_name_to_extract(env_vars, google_sheets_resource):
    ...
    with (
        patch(f"{_ING}.google_sheets.sheets_extract.extract", mock_extract),
        patch(f"{_ING}.google_sheets.gcs_load.load", return_value=FAKE_GCS_URI),
        patch(f"{_ING}.google_sheets.bq_load.load", return_value=FAKE_ROWS_LOADED),
        patch("dagster_gcp.GCSResource.get_client", return_value=MagicMock()),
        patch("dagster_gcp.BigQueryResource.get_client", return_value=MagicMock()),
