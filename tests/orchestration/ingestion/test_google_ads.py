...
from orchestration.defs.ingestion.google_ads import QUERY, TABLE, google_ads_raw
...
def test_materialize_gcs_source_is_table_name(env_vars, google_ads_resource):
    ...
    with (
        patch(f"{_ING}.google_ads.ads_extract.extract", return_value=SAMPLE_TABLE),
        patch(f"{_ING}.google_ads.gcs_load.load", mock_gcs_load),
        patch(f"{_ING}.google_ads.bq_load.load", return_value=FAKE_ROWS_LOADED),
        ...
        patch(
            f"{_ING}.resources.GoogleAdsResource.get_client",
...
def test_materialize_output_metadata_keys(env_vars, google_ads_resource):
    ...
    with (
        patch(f"{_ING}.google_ads.ads_extract.extract", return_value=SAMPLE_TABLE),
        patch(f"{_ING}.google_ads.gcs_load.load", return_value=FAKE_GCS_URI),
        patch(f"{_ING}.google_ads.bq_load.load", return_value=FAKE_ROWS_LOADED),
        patch("dagster_gcp.GCSResource.get_client", return_value=MagicMock()),
        patch("dagster_gcp.BigQueryResource.get_client", return_value=MagicMock()),
        patch(
...
def test_materialize_passes_customer_id_to_extract(env_vars, google_ads_resource):
    ...
    with (
        patch(f"{_ING}.google_ads.ads_extract.extract", mock_extract),
        patch(f"{_ING}.google_ads.gcs_load.load", return_value=FAKE_GCS_URI),
        patch(f"{_ING}.google_ads.bq_load.load", return_value=FAKE_ROWS_LOADED),
        patch("dagster_gcp.GCSResource.get_client", return_value=MagicMock()),
        patch("dagster_gcp.BigQueryResource.get_client", return_value=MagicMock()),
