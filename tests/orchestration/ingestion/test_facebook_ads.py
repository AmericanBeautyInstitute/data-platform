...
from orchestration.defs.ingestion.facebook_ads import TABLE, facebook_ads_raw
...
def test_materialize_gcs_source_is_table_name(env_vars, facebook_ads_resource):
    ...
    with (
        patch(
            f"{_ING}.facebook_ads.fb_extract.extract",
            return_value=SAMPLE_TABLE,
        ),
        patch(f"{_ING}.facebook_ads.gcs_load.load", mock_gcs_load),
        patch(f"{_ING}.facebook_ads.bq_load.load", return_value=FAKE_ROWS_LOADED),
        ...
        patch(
            f"{_ING}.resources.FacebookAdsResource.get_client",
...
def test_materialize_output_metadata_keys(env_vars, facebook_ads_resource):
    ...
    with (
        ...
        patch(f"{_ING}.facebook_ads.gcs_load.load", return_value=FAKE_GCS_URI),
        patch(f"{_ING}.facebook_ads.bq_load.load", return_value=FAKE_ROWS_LOADED),
        patch("dagster_gcp.GCSResource.get_client", return_value=MagicMock()),
        patch("dagster_gcp.BigQueryResource.get_client", return_value=MagicMock()),
        patch(
...
def test_materialize_passes_date_range_to_extract(env_vars, facebook_ads_resource):
    ...
    with (
        patch(f"{_ING}.facebook_ads.fb_extract.extract", mock_extract),
        patch(f"{_ING}.facebook_ads.gcs_load.load", return_value=FAKE_GCS_URI),
        patch(f"{_ING}.facebook_ads.bq_load.load", return_value=FAKE_ROWS_LOADED),
        patch("dagster_gcp.GCSResource.get_client", return_value=MagicMock()),
        patch("dagster_gcp.BigQueryResource.get_client", return_value=MagicMock()),
