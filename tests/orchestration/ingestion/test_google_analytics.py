...
from orchestration.defs.ingestion.google_analytics import (
    REPORT_CONFIG,
    TABLE,
...
def test_materialize_gcs_source_is_table_name(env_vars, google_analytics_resource):
    ...
    with (
        patch(
            f"{_ING}.google_analytics.ga_extract.extract",
            return_value=SAMPLE_TABLE,
        ),
        patch(f"{_ING}.google_analytics.gcs_load.load", mock_gcs_load),
        patch(
            f"{_ING}.google_analytics.bq_load.load",
        ...
        patch(
            f"{_ING}.resources.GoogleAnalyticsResource.get_client",
...
def test_materialize_output_metadata_keys(env_vars, google_analytics_resource):
    ...
    with (
        ...
        patch(f"{_ING}.google_analytics.gcs_load.load", return_value=FAKE_GCS_URI),
        patch(
            f"{_ING}.google_analytics.bq_load.load",
            return_value=FAKE_ROWS_LOADED,
        ),
...
def test_materialize_passes_date_range_to_extract(env_vars, google_analytics_resource):
    ...
    with (
        patch(f"{_ING}.google_analytics.ga_extract.extract", mock_extract),
        patch(f"{_ING}.google_analytics.gcs_load.load", return_value=FAKE_GCS_URI),
        patch(
            f"{_ING}.google_analytics.bq_load.load",
            return_value=FAKE_ROWS_LOADED,
